...
empty by default, since the Virtual-Fleet IoT identity above is still provisioning-pending, so the
...
- Sampled `TotalProcessorTime`/`WorkingSet64` every 300ms (Windows `Get-Process`) for all three
  ...
  - `world-server` (idle, no connected Colyseus clients): CPU time **flat at 1.062s to 1.109s** for
  ...
  - `loadtest` process itself (the actual 95-agent crowd-sim CPU cost, run unpaced/back-to-back,
    ...
    0.19s to 5.20s over 4.86s wall time, i.e. **~100% of one core saturated** when hammered with no
