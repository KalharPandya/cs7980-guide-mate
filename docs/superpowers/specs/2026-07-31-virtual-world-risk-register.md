...
the full task-by-task log).

**This document originally said that process gave "high confidence in the CODE's correctness."
2026-08-02 proved that overstated.** All of it passed while the product shipped a one-robot fleet,
a wrong-shaped map, three unreachable rooms, and a reachability test that could not detect
unreachability (Detour returns success on a PARTIAL path). The reviews were not lazy; they checked
the code against the spec and the tests, and the tests were themselves wrong or absent. Kalhar
found both headline defects in minutes by opening a browser. Read the rest of this register with
that calibration: green CI here means "no known contradiction", not "verified working". This
...
|---|---|---|---|
| 1 | **RESOLVED, AND IT WAS RIGHT.** This was logged as the project's #1 risk: every visual task was verified by tsc/build/network checks and live data polling, never by a human or AI actually watching the scene, because this sandbox's Browser pane never composites a frame (root cause: `document.hidden` is `true` and the R3F canvas stays at its un-laid-out 300x150 default; the WebGL context itself is valid). **On 2026-08-02 Kalhar opened it in a real browser and the risk paid out immediately.** Within minutes he reported two headline defects that the entire Phase 0-5 implementer + spec-review + quality-review process had passed clean: only ONE robot was moving (the production fleet spawn had never been written; `visitors.test.ts` masked it by spawning 50 test robots itself), and the floor plan did not match the real building (the map was a previous session's freehand approximation of source images that had been pasted into a chat and never committed). Both are now fixed and the source images are committed at `world/data/source/`. | Kalhar looked at it. That is the whole verification, and it beat every automated check this project had. | Keep doing it. The standing lesson, recorded in the progress log: **weight "did anyone actually watch it work" above any amount of green CI.** Re-open the browser after every significant map or fleet change, since neither defect was detectable from test output. |
...
| 3 | **World-server is not deployed to the LIVE instance, but the whole Docker/Compose stack is now proven, not just theorized.** Task 5.1's implementer and reviewer both hit "Docker daemon unreachable" in their sandboxes. Retried later in this same session (2026-07-31) after the daemon happened to become reachable: started Docker Desktop, ran a real `docker build -f world/Dockerfile .` (clean, no errors), then a full `docker compose -f compose.yaml up -d --build` bringing up all THREE real containers (`app`, `world-server`, `caddy`) together, throwaway local project name, no real secrets. Verified live: both `app` and `world-server` reported Docker-healthcheck `healthy`; `curl http://localhost/healthz` (app, via Caddy) and `curl http://localhost/world/healthz` (world-server, via Caddy's `/world/*` prefix-strip route) both returned real `200 {"ok":true}`; a real `POST /world/matchmake/joinOrCreate/world` through the full Caddy-proxied path returned a genuine Colyseus session/room id. Everything torn down and cleaned afterward (containers, volumes, network, test images all removed, confirmed empty). This is NOT the live `echo.kalhar.ca` instance, still not touched, but the exact same build/compose/routing mechanics Kalhar will run there are now proven to work, not just validated by `docker compose config` and reasoning about the Dockerfile. | Real `docker build`, real 3-container `docker compose up`, real HTTP round-trips through the real Caddy routing, real Colyseus join, all executed and observed directly, then fully torn down. | Kalhar still runs the actual deploy against `echo.kalhar.ca` himself (steps in `docs/agent-poc/access-ground-truth.md`), but with much higher confidence now that the mechanics themselves are correct, since the only new risk left there is instance-specific (real AWS creds, real DNS/TLS, real `t3.large` resource limits), not the container/compose wiring. |
| 4 | **t3.large co-tenancy load is only proxy-measured.** The design spec's own open risk ("FastAPI + Node world-server + Bedrock calls untested together") is still open. Task 5.1's numbers came from an 8-core/24GB dev box, explicitly caveated as much bigger than the real 2 vCPU/8GB t3.large. **2026-08-03 correction:** the previously-quoted "crowd-sim cost ~3.5% of one core" was `AgentCrowd.tick()` alone, not the real per-frame `WorldRoom.update()` cost (schema sync + the visitor/escort subsystem were never included) -- see `world/scripts/frametest.ts` (new, `npm run test:frame`) for the corrected full-frame measurement. | Combined idle RSS ~166MB. Full-frame `WorldRoom.update()` at the real 95-agent steady state, measured directly (not proxied via crowd-tick alone): avg 0.53ms, max 1.48ms, p95 0.64ms, 0/7500 ticks over the 16.6ms budget on the same oversized dev box; route-recomputation worst-case burst (all 50 robots re-tasked in one tick) adds +1.85ms, still under budget. | After Kalhar deploys (item 3), run `world/scripts/frametest.ts` (not just `loadtest.ts`) against the live instance while watching `docker stats` and the new CloudWatch alarm (`guidemate-poc-world-cpu`, 85% threshold) for a real number. |
...
| 7 | **SUPERSEDED 2026-08-02: the floor plan is no longer eyeballed.** This row used to say the coordinates were hand-approximated and accepted as good-enough. Kalhar checked the running demo against the real plan and rejected that: the map was the wrong SHAPE (an axis-aligned 36x21 rectangle with one core hole, versus the real pinwheel plate with angled wings and two service bands). The source images, pasted into a chat in July and never committed, were recovered from an old session transcript and are now committed at `world/data/source/` (original plus a higher-DPI scan). The map was re-traced algorithmically (OpenCV/Hough), then had doors moved onto their real thresholds, glass walls restored after a rebuild silently dropped them, and three misplaced rooms corrected. The metres-per-pixel scale, previously assumed, was independently derived from elevator shafts, core corridor width and stair-tread pitch, all three bracketing the existing value. | Algorithmic extraction from the committed source scan; geometry rendered back over the plan and visually compared; scale cross-checked by three independent standard-element measurements. | Remaining honest gap: absolute scale rests on assumed standard sizes (a shaft is ~2.0-2.4m, a tread ~0.28-0.30m), so it is confirmed to within roughly +/-10 percent, not surveyed. One real printed dimension from the building would pin it exactly. |
...
| 13 | **This worktree is edited by several live Claude sessions at once, and that has caused real damage.** Observed repeatedly on 2026-08-02: two sessions independently re-traced the same map, one session's in-progress edits vanished under another's commit, a subagent had its work superseded mid-task, and at the time of writing a concurrent session has uncommitted changes that leave the map at 17/18 rooms reachable (Gender Neutral Washroom 0.85m short, `DT_PARTIAL_RESULT` set). Nothing was permanently lost, but only because the collisions were noticed. | Detected by `git status`/`git log` changing under a running task, and by the strengthened reachability test catching the broken working-tree state. | Before editing a shared file, check `git status` and the file's mtime; if another session is live in it, wait rather than race. Never `git add -A`, never reset/stash/checkout/clean here. If the collisions get worse, give each session its own worktree instead of sharing one. |
