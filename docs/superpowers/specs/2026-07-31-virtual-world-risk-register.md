...
| 11 | **Adversarial security/robustness pass (2026-07-31, post-completion), two Minor findings, no fixes required before the demo.** (a) `WorldRoom.addAgent`/the fleet `assign` handler never check total agent count against `MAX_AGENTS=128` before calling the native Crowd's `addAgent`, implausible to hit at this demo's scale (Task 1.3 load-tested ~95 under budget) and the native library's at-capacity behavior is unvalidated, so worth a guard only if the simulated-visitor target is ever scaled up. (b) The emote-mirror feature (Task 5.3) has no rate limit and multiple virtual sessions could all target the same mirror robot concurrently, a real constraint only if the feature is heavily used at once, naturally throttled by per-turn LLM latency in practice. Everything else checked (untrusted MQTT/WS/HTTP input handling, auth on the new admin routes, information disclosure, the browser client's trust boundary for zero XSS vectors) came back clean, calibrated honestly against this being a small-audience class demo on an isolated sandbox AWS account, not a public product. | Full code read for every area listed, not assumed. | Neither finding blocks the demo. Add an agent-count guard and/or emote-mirror rate limit only if usage patterns at rehearsal suggest it's actually needed. |
