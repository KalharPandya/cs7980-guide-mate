...
| 14 | Phase 4 (Moses control, sim visitors, phone controller) | **in progress — Task 4.1 done** | Detailed task breakdown written 2026-07-31 (see implementation plan). Task 4.1 (sim visitor spawner + `requestGuide`) done, commit `1876b95`: spec review DONE, quality review "approved with minor follow-ups" (no blockers). Found and fixed a real deadlock bug mid-implementation (naive fixed-offset trailing point sat in the robot's own future path under Detour's local avoidance; fixed with a position-history "conga line" instead). Measured steady-state ~45 simulated visitors held (min 36-38/avg ~43/max 45 across runs), no double-assignment. **Deferred cleanup (non-blocking, fold into next visitors.ts touch or Phase 5 polish):** split `visitors.ts` (569 lines, 3 concerns: assignment/binding, trailing physics, spawner lifecycle) into `escortManager.ts` + `simulatedVisitorSpawner.ts`; import `WorldRoom.ts`'s `maxSpeed` constant instead of the duplicated `ASSUMED_ROBOT_SPEED_MPS` literal; extract a `freshEscortFields()` helper (3x duplicated reset). Task 4.2 (Moses tool + fleet assign command + bridge routing) next. |
