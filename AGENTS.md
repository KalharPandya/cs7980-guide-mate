...
- **No-motion bring-up:** `config/nav2_no_motion.yaml` (in the explorer pkg) + [`docs/mapping/bringup-no-motion.md`](docs/mapping/bringup-no-motion.md) — full stack on a docked robot with every Nav2 velocity zeroed. **Note:** can't be driven from the Codex Bash shell (its nodes aren't cross-discovered by the boot systemd nodes → can't wake the `auto_standby` lidar); run from the robot's terminal.
...
- Robot-specific findings that aren't derivable from code live in Codex memory (`MEMORY.md` index).
