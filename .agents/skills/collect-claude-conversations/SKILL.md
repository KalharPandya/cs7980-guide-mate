---
name: collect-Codex-conversations
description: Use when asked to collect, archive, snapshot, back up, or update the Codex conversation/session transcripts ("conversations with the cloud") for this repo — runs the configurable collector that slims the JSONL, redacts secrets, verifies no credentials leak, and commits the result.
---

# Collect Codex Conversations

Archive this project's Codex session transcripts into the repo as slimmed,
**secret-free** JSONL. Wraps `scripts/collect_claude_conversations.py`.

## When to use

- "collect / archive / back up / update our Codex conversations"
- "snapshot the AI sessions for this repo"
- "refresh the Codex-conversations archive"
- Periodically, to keep `Codex-conversations/sessions/` current.
...
│  │   └─ <YYYY-MM-DD>_<id8>.jsonl   #    one file per Codex session
...
└─ .Codex/skills/collect-Codex-conversations/SKILL.md   # this skill
```

The default output dir (`Codex-conversations/sessions/`) is resolved relative
...
1. Scans `~/.Codex/projects/**` (honors `CLAUDE_CONFIG_DIR`).
...
3. Writes slimmed JSONL to `Codex-conversations/sessions/<date>_<id8>.jsonl`:
...
5. **Commit & push.** Per the repo owner's global rule, the commit message MUST
   include "Kalhar" and MUST NOT reference Codex/AI:
   ```bash
   git add -A
   git commit -m "Kalhar: Update Codex conversation archive"
   git push origin main
   ```

## Configuring which directories are collected

Edit `Codex-conversations/collect.config.json`:
```json
...
See `Codex-conversations/README.md` for full details.
